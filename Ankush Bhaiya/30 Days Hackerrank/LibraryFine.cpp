...
int libraryFine(int d1, int m1, int y1, int d2, int m2, int y2) {
    int fineDay=0, fineYear=0, fineMonth=0;
    if (d1>=d2 && m1==m2 && y1==y2) {
        fineDay = (d1-d2) *15;
        cout<<"fine day " << fineDay<<endl;
    }
    if (m1>=m2 && y1==y2 ) {
        fineMonth = (m1-m2) *500;
        cout<<"fine month " << fineMonth<<endl;

    }
    if (y1>=y2) {
        fineYear = (y1-y2 ) *10000;
        cout<<"fine year " << fineYear<<endl;

    }
    return fineDay + fineMonth + fineYear;
